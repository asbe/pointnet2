...
def virtual_scan(xyz, mode=-1):
    ...
    if mode == -1:
        view_dr = np.array([2*np.pi*np.random.random(),
                            np.pi/10*(np.random.random()-0.75)])
        camloc[:2] -= (0.8+0.7*np.random.random()) * \
            np.array([np.cos(view_dr[0]), np.sin(view_dr[0])])
    else:
        view_dr = np.array([np.pi/4*mode, 0])
        camloc[:2] -= np.array([np.cos(view_dr[0]), np.sin(view_dr[0])])
    ct_ray_dr = np.array([np.cos(view_dr[1])*np.cos(view_dr[0]),
                          np.cos(view_dr[1])*np.sin(view_dr[0]), np.sin(view_dr[1])])
    ...
    xx = xx.reshape(-1, 1)
    yy = yy.reshape(-1, 1)
    rays = xx*hr_dr.reshape(1, -1)+yy * \
        vt_dr.reshape(1, -1)+ct_ray_dr.reshape(1, -1)
